...
static int initializeStreaming(json_t *obj)
{
    ...
    for (int i=0; i< numSources; i++)
    {
        ...
        if (strncmp(sourceType,"webcam",6) == 0)
        {
            ...
            obs_data_t *settings = obs_data_create();
                ...
                json_t *numeratorObj = json_object_get(sourceInfo, "fpsNumerator");
                if (!json_is_number(numeratorObj)) {
                    fprintf(stderr, "error: fpsNum is not a number\n");
                    return 0;
                }
                ...
                json_t *denominatorObj = json_object_get(sourceInfo, "fpsDenominator");
                if (!json_is_number(denominatorObj)) {
                    fprintf(stderr, "error: denominatorObj is not a number\n");
                    return 0;
                }
    ...
    for (int i=0;i < numStreamingScenes; i++) {
        ...
        for (int j=0; j< numSources; j++)
        {
            ...
            json_t* sourceTypeObj = json_object_get(itemSourceInfo, "type");
            const char* sourceType = json_string_value(sourceTypeObj);
            ...
            json_t *cropLeftObj = json_object_get(itemSourceInfo, "cropLeft");
            if (cropLeftObj) {
              cropLeft = json_integer_value(cropLeftObj);
            }
            json_t *cropRightObj = json_object_get(itemSourceInfo, "cropRight");
            if (cropRightObj) {
              cropRight = json_integer_value(cropRightObj);
            }
            json_t *cropTopObj = json_object_get(itemSourceInfo, "cropTop");
            if (cropTopObj) {
              cropTop = json_integer_value(cropTopObj);
            }
            json_t *cropBottomObj = json_object_get(itemSourceInfo, "cropBottom");
            if (cropBottomObj) {
              cropBottom = json_integer_value(cropBottomObj);
            }
            ...
            else if (strncmp(sourceType,"microphone",10) == 0)
            {
                ...
                if (item == NULL) {
                    blog(LOG_ERROR, "Could not add microphone item");
                } else {
                    blog(LOG_INFO, "Added microphone to scene");
                }
            }
        }
...
static const int startStreaming(json_t *command)
{
    ...
    obs_service_t *service = obs_service_create("rtmp_custom", "streaming_service", NULL, NULL);
    ...
    encoder = obs_video_encoder_create("obs_x264", "simple_h264_streaming", NULL, NULL);
    if (!encoder) {
        blog(LOG_ERROR, "ERROR MAKING ENCODER");
        return 1;
    }
    ...
    audioEncoder = obs_audio_encoder_create("ffmpeg_aac", "simple_aac_streaming", NULL, 0, NULL);
    if (!audioEncoder) {
        blog(LOG_ERROR, "ERROR MAKING ENCODER");
    }
    blog(LOG_INFO, "Created audio encoder");
